"""
P12 Transformer query-method sweep (natural training pool, weighted loss —
the exp_p12_transformer_baleval_weightedloss.py training setup) over BOTH
eval-split variants:
  p12_transformer_baleval — balanced 50/50 test+val (drains the pool)
  p12_transformer         — natural stratified split (~14% positive everywhere)
...
batchbald is cheap here (binary task, ~7.6k pool).

active=p12_med_bal (balanced initial labels, handoff reference config);
switch to p12_med for an imbalanced initial label seed.

25 query rows x 2 eval-split settings x 3 seeds = 150 runs.
"""
...
config_dict = {
    ...
    "query": queries,
    "data": [
        "p12_transformer_baleval",  # balanced 50/50 test+val
        "p12_transformer",          # natural stratified split
    ],
    "active": ["p12_med_bal"],
    "optim": ["adam"],
}
